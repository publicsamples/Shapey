...
using global_cable3_t_index = runtime_target::indexers::fix_hash<2333886>;
...
DECLARE_PARAMETER_RANGE_STEP(Dest_InputRange, 
                             1., 
                             3., 
                             1.);
DECLARE_PARAMETER_RANGE_STEP(Dest_0Range, 
                             0., 
                             2., 
                             1.);

using Dest_0 = parameter::from0To1<lfo_impl::branch_t, 
                                   0, 
                                   Dest_0Range>;

using Dest = parameter::chain<Dest_InputRange, Dest_0>;
...
template <int NV> struct instance: public lfo_impl::lfo_t_<NV>
{
	...
	struct metadata
	{
		...
		SNEX_METADATA_ENCODED_PARAMETERS(94)
		{
            ...
            0x9000, 0x0041, 0x7000, 0x0041, 0x8000, 0x003F, 0x8000, 0x5B3F, 
            0x0001, 0x0000, 0x6964, 0x0076, 0x0000, 0x3F80, 0x0000, 0x4180, 
            0x0000, 0x4150, 0x0000, 0x3F80, 0x0000, 0x3F80, 0x025B, 0x0000, 
            0x6D00, 0x646F, 0x0065, 0x0000, 0x3F80, 0x0000, 0x40A0, 0x0000, 
            0x3F80, 0x0000, 0x3F80, 0x0000, 0x3F80, 0x035B, 0x0000, 0x4400, 
            0x7365, 0x0074, 0x0000, 0x3F80, 0x0000, 0x4040, 0x0000, 0x4040, 
            0x0000, 0x3F80, 0x0000, 0x3F80, 0x045B, 0x0000, 0x4100, 0x6A64, 
            0x7473, 0x0000, 0x0000, 0x0000, 0x8000, 0xDF3F, 0x37F4, 0x003F, 
	...
	instance()
	{
		...
		auto& chain6 = this->getT(5).getT(2);                // lfo_impl::chain6_t
		auto& peak4 = this->getT(5).getT(2).getT(0);         // lfo_impl::peak4_t
		auto& global_cable3 = this->getT(5).getT(2).getT(1); // routing::global_cable<global_cable3_t_index, parameter::empty>
		...
		this->setParameterT(0, 15.);
		this->setParameterT(1, 13.);
		this->setParameterT(2, 1.);
		this->setParameterT(3, 3.);
		this->setParameterT(4, 0.71858);
	...
	void connectToRuntimeTarget(bool addConnection, const runtime_target::connection& c)
	{
		...
		this->getT(5).getT(2).getT(1).connectToRuntimeTarget(addConnection, c); // routing::global_cable<global_cable3_t_index, parameter::empty>
	}
	...
	void setExternalData(const ExternalData& b, int index)
	{
		...
		this->getT(5).getT(2).getT(0).setExternalData(b, index); // lfo_impl::peak4_t
	}
};
}
