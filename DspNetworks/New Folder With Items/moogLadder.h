...
#include "src/moogLadder.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct moogLadderMetaData {
		SN_NODE_ID("moogLadder");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using moogLadder = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _moogLadder, moogLadderMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
