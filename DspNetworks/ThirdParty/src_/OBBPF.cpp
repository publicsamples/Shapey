...
Compilation options: -lang cpp -rui -nvi -ct 1 -cn _OBBPF -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0
------------------------------------------------------------ */
...
static float _OBBPF_faustpower2_f(float value) {
	return value * value;
}

struct _OBBPF final : public ::faust::dsp {
	...
	void metadata(Meta* m) { 
		m->declare("compile_options", "-lang cpp -rui -nvi -ct 1 -cn _OBBPF -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
		m->declare("filename", "OBBPF.dsp");
	...
	void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("OBBPF");
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp3 = fTemp2 * (1.0f - 0.33333334f * _OBBPF_faustpower2_f(fTemp2));
			float fRec0 = fTemp3;
			fRec1[0] = fRec1[1] + fSlow4 * fTemp3;
			fRec2[0] = fTemp1 + fTemp3;
			output0[i0] = FAUSTFLOAT(fRec0);
			float fTemp4 = float(input1[i0]) - (fRec4[1] + fSlow1 * fRec5[1]);
			float fTemp5 = fSlow3 * fTemp4;
			float fTemp6 = std::max<float>(-1.0f, std::min<float>(1.0f, fRec5[1] + fTemp5));
			float fTemp7 = fTemp6 * (1.0f - 0.33333334f * _OBBPF_faustpower2_f(fTemp6));
			float fRec3 = fTemp7;
			fRec4[0] = fRec4[1] + fSlow4 * fTemp7;
			fRec5[0] = fTemp5 + fTemp7;
	...
	#define FAUST_FILE_NAME "OBBPF.dsp"
	#define FAUST_CLASS_NAME "_OBBPF"
	#define FAUST_COMPILATION_OPIONS "-lang cpp -rui -nvi -ct 1 -cn _OBBPF -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0"
