...
struct _gran final : public ::faust::dsp {
	...
	void metadata(Meta* m) { 
		...
		m->declare("hoa.lib/author", "Wargreen");
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow6 = std::max<float>(-24.0f, std::min<float>(24.0f, float(fHslider4)));
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp22 = ftbl1[iTemp13] * fTemp15 + ftbl1[iTemp10] * fTemp9 + ftbl1[iTemp18] * fTemp20 + ftbl1[iTemp4] * fTemp3;
			fVec5[0] = fTemp22;
