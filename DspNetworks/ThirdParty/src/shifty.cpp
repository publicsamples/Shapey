...
Compilation options: -lang cpp -rui -nvi -ct 1 -cn _shifty -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0
------------------------------------------------------------ */

#ifndef  ___shifty_H__
#define  ___shifty_H__
...
struct _shifty final : public ::faust::dsp {
	...
	void metadata(Meta* m) { 
		m->declare("compile_options", "-lang cpp -rui -nvi -ct 1 -cn _shifty -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
		m->declare("delays.lib/name", "Faust Delay Library");
		m->declare("delays.lib/version", "1.1.0");
		m->declare("filename", "shifty.dsp");
	...
	void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("shift (semitones)", &fHslider1, FAUSTFLOAT(0.0f), FAUSTFLOAT(-1.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.01f));
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow1 = std::pow(2.0f, 0.083333336f * std::max<float>(-1.0f, std::min<float>(1.0f, float(fHslider1))));
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			output1[i0] = FAUSTFLOAT(fTemp8 * (fVec1[(IOTA0 - iTemp7) & 131071] * fTemp6 + fVec1[(IOTA0 - iTemp5) & 131071] * fTemp2) + fTemp16 * (fTemp13 * fVec1[(IOTA0 - iTemp11) & 131071] + fTemp14 * fVec1[(IOTA0 - iTemp15) & 131071]));
			fRec0[1] = fRec0[0];
			IOTA0 = IOTA0 + 1;
		}
	}
	...
	#define FAUST_FILE_NAME "shifty.dsp"
	#define FAUST_CLASS_NAME "_shifty"
	#define FAUST_COMPILATION_OPIONS "-lang cpp -rui -nvi -ct 1 -cn _shifty -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0"
	#define FAUST_INPUTS 2
	#define FAUST_OUTPUTS 2
	#define FAUST_ACTIVES 3
	#define FAUST_PASSIVES 0

	FAUST_ADDHORIZONTALSLIDER("Pitch Shifter/shift (semitones)", fHslider1, 0.0f, -1.0f, 1.0f, 0.01f);
	...
	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, shift_(semitones), "Pitch Shifter/shift (semitones)", fHslider1, 0.0f, -1.0f, 1.0f, 0.01f) \
