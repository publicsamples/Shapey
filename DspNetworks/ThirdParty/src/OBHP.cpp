...
Compilation options: -lang cpp -rui -nvi -ct 1 -cn _OBHP -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0
------------------------------------------------------------ */
...
static float _OBHP_faustpower2_f(float value) {
	return value * value;
}

struct _OBHP final : public ::faust::dsp {
	...
	void metadata(Meta* m) { 
		m->declare("compile_options", "-lang cpp -rui -nvi -ct 1 -cn _OBHP -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
		m->declare("filename", "OBHP.dsp");
	...
	void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("OBHP");
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow1 = 1.0f / std::max<float>(1.0f, std::min<float>(1e+01f, float(fHslider1))) + fSlow0;
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp2 = std::max<float>(-1.0f, std::min<float>(1.0f, fTemp1 + fRec2[1]));
			float fTemp3 = fTemp2 * (1.0f - 0.33333334f * _OBHP_faustpower2_f(fTemp2));
			float fTemp4 = fSlow0 * fTemp3 + fRec1[1];
			float fRec0 = fTemp4;
			fRec1[0] = fSlow4 * fTemp3 + fRec1[1];
			fRec2[0] = fTemp1 + fTemp3;
			output0[i0] = FAUSTFLOAT(fRec0);
			float fTemp5 = float(input1[i0]) - (fRec4[1] + fSlow1 * fRec5[1]);
			float fTemp6 = fSlow3 * fTemp5;
			float fTemp7 = std::max<float>(-1.0f, std::min<float>(1.0f, fRec5[1] + fTemp6));
			float fTemp8 = fTemp7 * (1.0f - 0.33333334f * _OBHP_faustpower2_f(fTemp7));
			float fTemp9 = fSlow0 * fTemp8;
			float fRec3 = fRec4[1] + fTemp9;
			fRec4[0] = fRec4[1] + fSlow4 * fTemp8;
			fRec5[0] = fTemp6 + fTemp8;
	...
	#define FAUST_FILE_NAME "OBHP.dsp"
	#define FAUST_CLASS_NAME "_OBHP"
	#define FAUST_COMPILATION_OPIONS "-lang cpp -rui -nvi -ct 1 -cn _OBHP -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0"
