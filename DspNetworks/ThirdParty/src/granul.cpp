...
struct _granul final : public ::faust::dsp {
	...
	void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->declare(&fHslider0, "BELA", "ANALOG_2");
		ui_interface->addHorizontalSlider("Probability", &fHslider0, FAUSTFLOAT(7e+01f), FAUSTFLOAT(5e+01f), FAUSTFLOAT(1e+02f), FAUSTFLOAT(1.0f));
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow0 = std::max<float>(5e+01f, std::min<float>(1e+02f, float(fHslider0)));
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			iVec4[0] = iTemp3;
			fRec3[0] = float((iTemp3 - iVec4[1]) > 0) + fRec3[1] - fSlow2 * float(fRec3[1] > 0.0f);
	...
	FAUST_ADDHORIZONTALSLIDER("Granulator/Granulator/Probability", fHslider0, 7e+01f, 5e+01f, 1e+02f, 1.0f);
	...
	#define FAUST_LIST_ACTIVES(p) \
		...
		p(HORIZONTALSLIDER, Probability, "Granulator/Granulator/Probability", fHslider0, 7e+01f, 5e+01f, 1e+02f, 1.0f) \
